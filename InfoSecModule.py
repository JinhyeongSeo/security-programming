...
def rsaEncrypt(message, publicKey):
    ...
    rsaEncObj = PKCS1_OAEP.new(pubKey)     # (n, e) 추출된 것을 이용해서 RSA 암호화 객체 생성
...
def rsaDecrypt(received, privateKey):
    priKey = RSA.importKey(privateKey)    # rivateKey 바이트 스트림 --> (n, d) 추출...
...
def aesDecWithHash(key, iv, received):
    ...
    hash1 = output2[0:40]
    hash2 = SHA1.new(output2[40:])
    if hash1 == hash2.hexdigest().encode():
        print("Hash Verification OK!!!")
        return output2[40:]  ### modified
    else:
